...
class Bezier implements IDisposable {
  ...
  public static function point_on_quadratic_curve(t:Float, ax:Float, ay:Float, bx:Float, by:Float, cx:Float, cy:Float):Vector2 {
    ...
    inline function pos(a:Float, b:Float, c:Float) return a * tt + b * 2 * tu + c * uu;
  ...
  public static function point_on_cubic_curve(t:Float, ax:Float, ay:Float, bx:Float, by:Float, cx:Float, cy:Float, dx:Float, dy:Float):Vector2 {
    ...
    inline function pos(a:Float, b:Float, c:Float, d:Float) return a * ttt + b * 3 * tt * u + c * 3 * t * uu + d * uuu;
   ...
   * @param start_index Determines the starting control point that will be used to construct the Curve.
   */
  public function get_point(t:Float, start_index:Int = 0):Null<Vector2> {
    if (curve_count < 1 || start_index + curve_mode >= control_points.length) return null;
  ...
  inline function update_curve_count() {
    curve_count = Math.floor(control_points.length / curve_mode);
  }
